...
// T้his is a transition class for animating a custom Popup Viewcontroller
final class BottomSheetPresentationController: UIPresentationController {
    ...
    @objc private func onPan(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            // y change need to be over 10% to update the UI
            if panToDismissEnabled && progress > 0.1 && !presentedViewController.isBeingDismissed {
                presentingViewController.dismiss(animated: true)
            }
...
extension BottomSheetInteractiveDismissalTransition: UIViewControllerAnimatedTransitioning {
    ...
    func interruptibleAnimator(
        ...
        guard let offsetAnimator = offsetAnimator else {
            fatalError("Animator was not set")
        }
